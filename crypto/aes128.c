...
static void mixcolumns(uint8_t buf[16])
{
	...
	for (uint8_t i = 0; i < 16; i += 4) {
		...
		t[2] = buf[i + 2];
		t[3] = buf[i + 3];
		buf[i + 0] = GF2[t[0]] ^ GF3[t[1]] ^ t[2] ^ t[3];
		buf[i + 1] = GF2[t[1]] ^ GF3[t[2]] ^ t[3] ^ t[0];
		buf[i + 2] = GF2[t[2]] ^ GF3[t[3]] ^ t[0] ^ t[1];
		buf[i + 3] = GF2[t[3]] ^ GF3[t[0]] ^ t[1] ^ t[2];
	}
}
...
static void mixcolumns_inv(uint8_t buf[16])
{
	...
	for (uint8_t i = 0; i < 16; i += 4) {
		...
		t[2] = buf[i + 2];
		t[3] = buf[i + 3];
		buf[i + 0] = GF14[t[0]] ^ GF11[t[1]] ^ GF13[t[2]] ^ GF9[t[3]];
		buf[i + 1] = GF14[t[1]] ^ GF11[t[2]] ^ GF13[t[3]] ^ GF9[t[0]];
		buf[i + 2] = GF14[t[2]] ^ GF11[t[3]] ^ GF13[t[0]] ^ GF9[t[1]];
		buf[i + 3] = GF14[t[3]] ^ GF11[t[0]] ^ GF13[t[1]] ^ GF9[t[2]];
	}
}
...
void aes128_keyexpansion(const uint32_t key[4], uint32_t rk[44])
{
	...
	for (int i = 4; i < 44; i += 4) {
		rk[i] = rk[i - 4] ^ subword(rotword(rk[i - 1])) ^ rc[i / 4 - 1];
		rk[i + 1] = rk[i - 3] ^ rk[i];
