...
static inline void outputStr(char *buf, int *j, int nbytes, const char *s, char padding,
			     int padd_width)
{
	int len;
...
static inline void outputNum(char *buf, int *j, int nbytes, unsigned long long num, bool neg,
			     int base, char padding, int padd_width)
{
	char nbuf[64];
	nbuf[63] = '\0';
	int i = 63;
...
int vsnprintf(char *buf, size_t nbytes, const char *format, va_list args)
{
	...
	while (format[f] != '\0' && i < nbytes) {
		...
		case 'u':
			outputNum(buf, &i, nbytes, 0xFFFFFFFF & va_arg(args, int), false, 10,
				  padding, padd_width);
			break;
		case 'o':
			outputNum(buf, &i, nbytes, 0xFFFFFFFF & va_arg(args, int), false, 8,
				  padding, padd_width);
			break;
		case 'x':
			outputNum(buf, &i, nbytes, 0xFFFFFFFF & va_arg(args, int), false, 16,
				  padding, padd_width);
			break;
		case 's':
		...
		case 'l':
			...
			case 'u':
				outputNum(buf, &i, nbytes, va_arg(args, unsigned long), false, 10,
					  padding, padd_width);
				break;
			case 'o':
				outputNum(buf, &i, nbytes, va_arg(args, unsigned long), false, 8,
					  padding, padd_width);
				break;
			case 'x':
				outputNum(buf, &i, nbytes, va_arg(args, unsigned long), false, 16,
					  padding, padd_width);
				break;
			default:
